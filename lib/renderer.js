...
export function assembleResumeSections(html, layoutSpec) {
  const wrapped = `<div class="dsh-resume-root">${html}</div>`
  ...
  for (const part of parts) {
    if (part.startsWith('<h2>')) {
      ...
      const block = byHeading.get(heading) || byId.get(heading) || { id: `section-${sectionParts.size + 1}`, type: 'custom-section', source: heading }
      ...
      sectionParts.set(block.id, section)
      headerDone = true
    } else if (!headerDone) {
  ...
  if (!layout) return `<div class="dsh-resume-root">${rebuilt.join('\n')}</div>`
  const ordered = []
  const used = new Set()
  for (const id of Object.values(layout.regions).flat()) {
    const section = sectionParts.get(id)
    if (section && !used.has(id)) {
      ordered.push(section)
      used.add(id)
    }
  }
  for (const section of sectionParts.values()) {
    if (![...used].some((id) => section.includes(`data-module-id="${id}"`))) ordered.push(section)
  }
  const header = rebuilt.find((item) => item.startsWith('<header'))
  return `<div class="dsh-resume-root">${[header, ...ordered].filter(Boolean).join('\n')}</div>`
}
...
${bodyHtml}
    ...
    <div class="dsh-resume-layout-status" role="status" aria-live="polite">正在计算页数…</div>
  </main>
  <script>
  (() => {
    ...
    tuningStyle.textContent = [
      ...
      '.resume-document.template-terminal .dsh-resume-page-content{background:linear-gradient(90deg,#fff 0,#fff 96%,#f1f5f9 96%) !important;}',
    ...
    const indicator = document.querySelector('.dsh-resume-layout-status');
    if (indicator) {
...
export async function renderPreviewHtml(root, options = {}) {
  ...
  const bodyHtml = assembleResumeSections(markdownToHtml(md), layoutSpec)
