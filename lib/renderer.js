...
function renderModuleBody(type, html) {
  if (type === 'skill-tags') {
...
export function assembleResumeSections(html, layoutSpec) {
  ...
  for (const [index, part] of parts.entries()) {
    if (part.startsWith('<h2>')) {
      ...
      const sectionBody = renderModuleBody(block.type, part)
      const section = `<section class="dsh-resume-section ${moduleClass(block.type)}" data-module-id="${escapeHtml(block.id)}" data-module-type="${escapeHtml(block.type)}">${sectionBody}</section>`
...
<body>
  <main class="resume-document template-${escapeHtml(normalizedTemplate.visual.variant)}" data-source="${escapeHtml(sourcePath)}" data-template="${escapeHtml(templatePath)}" data-preview-path="${escapeHtml(previewPath || '')}" data-template-id="${escapeHtml(normalizedTemplate.id)}">
...
${bodyHtml}
  ...
  (() => {
    ...
    tuningStyle.textContent = [
      ...
      '.dsh-skill-tag{display:inline-flex;align-items:center;padding:3px 8px;border:1px solid color-mix(in srgb,' + template.visual.accentColor + ' 26%,#dbe3ef);border-radius:999px;font-size:.92em;line-height:1.35;}',
      '.dsh-timeline{border-left:2px solid color-mix(in srgb,' + template.visual.accentColor + ' 28%,transparent);padding-left:14px;}',
      '.dsh-metric-row{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;}',
      '.dsh-portfolio-card{padding:10px 12px;border:1px solid color-mix(in srgb,' + template.visual.accentColor + ' 20%,#e2e8f0);border-radius:' + template.visual.cornerRadius + 'px;background:color-mix(in srgb,' + template.visual.backgroundColor + ' 92%,#f8fafc);}',
