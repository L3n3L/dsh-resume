...
${bodyHtml}
  ...
  (() => {
    ...
    const pageMetrics = pages.map(({ content, page }, index) => {
      const usedHeight = Math.min(content.scrollHeight, content.clientHeight);
      const moduleNames = [...content.querySelectorAll('.resume-module > h2')].map((node) => node.textContent?.trim()).filter(Boolean);
      return {
        page: index + 1,
        usedHeight,
        blankRatio: Math.max(0, Number((1 - usedHeight / Math.max(1, content.clientHeight)).toFixed(3))),
        modules: moduleNames,
