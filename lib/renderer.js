...
export function assembleResumeSections(html) {
  ...
  for (const part of parts) {
    if (part.startsWith('<h2>')) {
      rebuilt.push(`<section class="dsh-resume-section">${part}</section>`)
      headerDone = true
    } else if (!headerDone) {
  ...
  return `<div class="dsh-resume-root">${rebuilt.join('\n')}</div>`
}
...
export async function renderPreviewHtml(root, options = {}) {
  ...
  const bodyHtml = assembleResumeSections(markdownToHtml(md))
