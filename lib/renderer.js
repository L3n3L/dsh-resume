...
function renderIconTokens(html, iconState = { next: 0 }) {
  ...
  return String(html || '').replace(/\[icon:([a-z0-9_-]+)\][ \t]*/gi, (match, rawName) => {
    ...
    if (definition?.svg) {
      ...
      return `<span class="dsh-icon dsh-icon-${name}" data-icon-name="${name}" data-icon-index="${index}" role="img" aria-label="${definition.label}"><svg viewBox="${svg.viewBox}" width="1em" height="1em" aria-hidden="true" focusable="false"><path d="${svg.d}" fill="${fill}"/></svg></span>`
    }
    if (definition) {
      return `<span class="dsh-icon dsh-icon-${name}" data-icon-name="${name}" data-icon-index="${index}" role="img" aria-label="${definition.label}"><span aria-hidden="true">${definition.glyph}</span></span>`
    }
    ...
    return `<span class="dsh-icon dsh-icon-unknown" data-icon-name="${name}" data-icon-index="${index}" role="img" aria-label="未注册图标"><span aria-hidden="true">•</span></span>`
  })
}
...
${bodyHtml}
  ...
  (() => {
    ...
    if (!isThumbnail) {
      ...
      // Keep the parent message for instant DSH UI updates. Also persist from
      // the preview itself when it is served by DSH, so MCP metrics do not
      // depend on a particular parent listener or iframe timing.
      ...
      if (window.location.protocol === 'http:' || window.location.protocol === 'https:') {
        void fetch('/dsh-resume/api/metrics', {
          ...
          if (!response.ok) throw new Error('metrics POST failed with HTTP ' + response.status);
        }).catch((error) => {
          window.parent?.postMessage({
            source: 'dsh-resume-metrics-error',
            previewRoot: metricPayload.previewRoot,
            previewPath: metricPayload.previewPath,
            renderId: metricPayload.renderId,
            contentHash: metricPayload.contentHash,
...
export async function renderPreviewHtml(root, options = {}) {
  ...
  const html = buildPreviewDocument({
    ...
    templateSpec,
    layoutSpec,
    initialIconTuning: options.initialIconTuning || {},
  })
...
async function renderPreviewUnlocked(root, options = {}) {
...
export async function renderPreview(root, options = {}) {
  return withWorkspaceLock(root, () => renderPreviewUnlocked(root, options))
}

// Used by callers that already hold the workspace lock so a save and its
// subsequent render remain one transaction.
export { renderPreviewUnlocked }
