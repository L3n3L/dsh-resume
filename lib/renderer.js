  ...
  for (const [index, part] of parts.entries()) {
    if (part.startsWith('<h2>')) {
      ...
      const sectionBody = shouldComposeEntries
        ? renderModuleBody(block.type, composeEntryRows(decorateEntryMarkup(renderIconTokens(part)), templateSpec.composition, blockSource || block.id), block.options, context)
      ...
      const headerSpecAttributes = pageSpec.header
        ...
        : ''
      sectionParts.header = `<header class="${headerClass}"${heroLayout}${headerSpecAttributes}>${decorateHeroHeader(renderIconTokens(part))}</header>`
      headerDone = true
    }
  }
