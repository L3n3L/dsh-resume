...
export function assembleResumeSections(html, layoutSpec, templateLayout = null, templateSpec = {}, context = {}) {
  ...
  for (const [index, part] of parts.entries()) {
    if (part.startsWith('<h2>')) {
      ...
      const sectionBody = decorateEntryMarkup(renderIconTokens(renderModuleBody(block.type, part, block.options, context)))
      ...
      sectionParts.header = `<header class="${headerClass}">${renderIconTokens(part)}</header>`
      headerDone = true
    }
  }
