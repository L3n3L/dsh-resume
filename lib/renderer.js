...
const SAFE_ICON_TOKENS = Object.freeze({
  github: { label: 'GitHub', glyph: 'GH' },
  linkedin: { label: 'LinkedIn', glyph: 'in' },
  react: { label: 'React', glyph: '⚛' },
  typescript: { label: 'TypeScript', glyph: 'TS' },
...
function renderIconTokens(html) {
  return String(html || '').replace(/\[icon:(github|linkedin|react|typescript|python|email|phone|link)\]/gi, (_match, rawName) => {
    const name = rawName.toLowerCase()
    const icon = SAFE_ICON_TOKENS[name]
    return `<span class="dsh-icon dsh-icon-${name}" role="img" aria-label="${icon.label}"><span aria-hidden="true">${icon.glyph}</span></span>`
  })
}
...
function composeEntryRows(html, composition = {}) {
  if (composition.entry !== 'timeline' || !/<h3\b/i.test(html)) return html
  return String(html).replace(/(<h3[^>]*>[\s\S]*?<\/h3>)([\s\S]*?)(?=<h3\b|$)/gi, (_match, title, tail) => {
    ...
    const metaMarkup = metaParts && metaBody?.length > 1
      ...
      : meta
    return `<div class="dsh-entry-row" data-entry-layout="${escapeHtml(composition.meta || 'split')}"><div class="dsh-entry-main">${titleMarkup}${context}</div><div class="dsh-entry-meta-slot">${metaMarkup}</div><div class="dsh-entry-detail">${detail}</div></div>`
  })
}
  ...
  for (const [index, part] of parts.entries()) {
    if (part.startsWith('<h2>')) {
      ...
      const sectionBody = shouldComposeEntries
        ? renderModuleBody(block.type, composeEntryRows(decorateEntryMarkup(renderIconTokens(part)), templateSpec.composition), block.options, context)
        : decorateEntryMarkup(renderIconTokens(renderModuleBody(block.type, part, block.options, context)))
      const compositionSection = templateSpec.composition?.section
        ? `dsh-section-composition-${String(templateSpec.composition.section).replace(/[^a-z0-9-]/gi, '-')}`
        : ''
      const compositionSkills = [block.id, block.source, block.type].some((value) => value === 'skills') && templateSpec.composition?.skills
        ? `dsh-skills-composition-${String(templateSpec.composition.skills).replace(/[^a-z0-9-]/gi, '-')}`
        : ''
      const classes = [moduleClass(block.type), presetClass(block.options), compositionSection, compositionSkills].filter(Boolean).join(' ')
      ...
      const compositionHeader = templateSpec.composition?.header
        ? `dsh-header-composition-${String(templateSpec.composition.header).replace(/[^a-z0-9-]/gi, '-')}`
        : ''
      ...
      sectionParts.header = `<header class="${headerClass}"${heroLayout}>${decorateHeroHeader(renderIconTokens(part))}</header>`
      headerDone = true
    }
  }
