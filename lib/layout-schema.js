...
const HERO_LAYOUTS = new Set(['inline', 'stacked', 'centered'])
...
function normalizeHero(raw) {
  ...
  return {
    type: 'hero',
    layout: HERO_LAYOUTS.has(raw.layout) ? raw.layout : 'inline',
    avatar: raw.avatar === 'right' || raw.avatar === 'none' ? raw.avatar : 'left',
  }
}

function withHero(node, raw) {
  const hero = normalizeHero(raw?.hero)
  return hero ? { ...node, hero } : node
}
...
function normalizeIr(raw, spec, validIds) {
  ...
  if (type === 'stack') return withHero({ type, items: validItemIds(raw.items, validIds) }, raw)
  if (type === 'grid') {
    return withHero({
      type,
  ...
  return withHero({ type, items: validItemIds(raw.items, validIds) }, raw)
}
...
export function validateLayoutSpec(input) {
  ...
  if (spec.ir?.hero && spec.ir.hero.type !== 'hero') errors.push('ir.hero.type must be hero')
  if (spec.ir?.hero && !HERO_LAYOUTS.has(spec.ir.hero.layout)) errors.push('ir.hero.layout is invalid')
