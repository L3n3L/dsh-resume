...
function renderSplitSidebar({ renderer, header, ordered, layout }) {
  ...
  const items = ordered.map((item) => renderItem(item, sideSet.has(item.id) ? 'dsh-column-side-item' : 'dsh-column-main-item')).join('\n')
  return `<div class="dsh-resume-root dsh-renderer-${renderer}">${header}${items}</div>`
}
