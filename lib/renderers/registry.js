...
function renderSingleColumnItem(item, pageSpec) {
  ...
  return `<div class="${className}" data-module-source="${String(item.sourceId || item.type || '').replace(/[^a-z0-9-]/gi, '-')}"${variantAttr}>${renderItem(item)}</div>`
}
...
function renderComposedLayout({ renderer, template, header, ordered, layout }) {
  ...
  return `<div class="dsh-resume-root dsh-renderer-${renderer} dsh-composed-layout${pageSpec ? ' dsh-single-column-page' : ''}"${specAttrs}>${header}<div class="dsh-composed-modules dsh-layout-stack" data-layout-type="stack">${modules}</div></div>`
}
