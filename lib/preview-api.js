...
export function registerPreviewRoutes(ctx) {
  ...
  disposers.push(ctx.webServer.register({
    kind: 'exact',
    path: '/dsh-resume/api/export/html2canvas.js',
    async handler(req, res) {
      try {
        if (req.method !== 'GET') {
        ...
        const content = await fs.readFile(new URL('../node_modules/html2canvas/dist/html2canvas.min.js', import.meta.url))
        res.writeHead(200, {
    ...
    async handler(req, res) {
      try {
        ...
        if (body.action === 'save') {
          ...
          return sendJson(res, 200, { saved: true, ...(await withWorkspaceLock(root, () => saveTemplate(root, parsed, { replaceExisting: body.replaceExisting === true }))) })
        }
        if (body.action === 'copy') {
