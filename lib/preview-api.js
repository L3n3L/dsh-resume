...
export function registerPreviewRoutes(ctx, options = {}) {
  const disposers = []
  const chooseWorkspaceDirectory = options.pickWorkspaceDirectory || pickWorkspaceDirectory
    ...
    async handler(req, res) {
      try {
        ...
        if (requestedAction === 'pick') {
          const picked = await chooseWorkspaceDirectory(root)
          if (!picked) return sendJson(res, 200, { action: 'pick', cancelled: true, ...(await getWorkspaceInfo(root)), defaultWorkspace: await getWorkspaceInfo(resolveJobhuntRoot(undefined)), recentWorkspaces: await getRecentWorkspaceInfos() })
          const resolvedInput = await resolveWorkspaceInput(picked)
