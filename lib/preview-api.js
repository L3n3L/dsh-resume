...
export function getLatestMetrics() {
  return previewState.metrics || {
    available: false,
    status: 'pending',
    retryable: true,
    renderId: previewState.renderId,
    previewPath: previewState.previewRel,
...
export function rememberPreview(root, previewRel) {
  ...
  previewState.renderId = randomUUID()
  ...
  disposers.push(ctx.webServer.register({
    ...
    async handler(_req, res) {
      try {
        ...
        const currentPreview = previewState.previewRel || previews[0] || null
        if (!previewState.previewRel && currentPreview) {
          previewState.root = root
          previewState.previewRel = currentPreview
          previewState.renderId = randomUUID()
        }
        sendJson(res, 200, {
          root,
          ...
          previewUrl: currentPreview
            ? `/dsh-resume/preview?path=${encodeURIComponent(currentPreview)}`
            : null,
          previews,
        })
      } catch (err) {
    ...
    async handler(req, res) {
      try {
        if (req.method === 'POST') {
          ...
          const metricPreview = String(body.previewPath || body.preview || '').replace(/\\/g, '/')
          if (previewState.previewRel && metricPreview && metricPreview !== previewState.previewRel) {
            return sendJson(res, 409, {
              error: 'metrics belong to a different preview',
              expectedPreview: previewState.previewRel,
              receivedPreview: metricPreview,
            })
          }
          previewState.metrics = {
            renderId: previewState.renderId,
            previewRel: metricPreview || previewState.previewRel,
            metrics: body.metrics,
            updatedAt: new Date().toISOString(),
          }
        }
        sendJson(res, 200, { ...previewState.metrics, previewRel: previewState.metrics?.previewRel || previewState.previewRel })
      } catch (err) {
        sendJson(res, 400, { error: String(err?.message || err) })
      }
    },
