...
import { applyPresentationOverride, getPresentationOverride, loadPresentation, presentationWithOverride, savePresentationOverride } from './presentation.js'
...
function workspaceBindingsPath() {
  ...
  if (dshHome) return path.join(dshHome, 'dsh-resume', 'workspace-bindings.json')

  // DSH_HOME is optional in the desktop/web launcher. Keep the selected
  // workspace persistent in the platform user-data directory instead of
  // silently falling back to in-memory state on every DSH restart.
  const userDataRoot = process.platform === 'win32'
    ? (process.env.LOCALAPPDATA || process.env.APPDATA || '')
    : (process.env.XDG_STATE_HOME || (process.env.HOME ? path.join(process.env.HOME, '.local', 'state') : ''))
  return userDataRoot ? path.join(userDataRoot, 'dsh-resume', 'workspace-bindings.json') : null
}
...
function renderKey(renderId, contentHash) {
  return `${String(renderId || '')}::${String(contentHash || '')}`
}

function metricEntry(state, { renderId, contentHash } = {}) {
  if (!state) return null
  const requestedRenderId = String(renderId || '')
  const requestedContentHash = String(contentHash || '')
  const currentMatches = (!requestedRenderId || state.renderId === requestedRenderId)
    && (!requestedContentHash || state.contentHash === requestedContentHash)
  if (currentMatches) return state
  if (!requestedRenderId && !requestedContentHash) return null
  return state.renderHistory?.get(renderKey(requestedRenderId, requestedContentHash)) || null
}

function pendingMetrics(state) {
  return {
    available: false,
    status: 'pending',
    retryable: true,
...
export function getLatestMetrics(root, previewRel, identity) {
  const state = requestState(root, previewRel)
  const entry = metricEntry(state, identity)
  return entry?.metrics || pendingMetrics(entry || state)
}

export function rememberPreview(root, previewRel, rendered = {}, { activate = true } = {}) {
  ...
  // A normal preview GET may render the same effective document with a new
  // random renderer id (for example when layout query parameters are present).
  // Reusing the identity for an unchanged content hash prevents the client
  // status poll from mistaking its own iframe load for an external MCP write.
  if (previous && rendered.contentHash && previous.contentHash === rendered.contentHash) {
    if (activate) activePreviewKey = key
    return previous
  }
  const renderHistory = new Map(previous?.renderHistory || [])
  if (previous?.renderId && previous?.contentHash) {
    renderHistory.set(renderKey(previous.renderId, previous.contentHash), {
      root: previous.root,
      previewRel: previous.previewRel,
      renderId: previous.renderId,
      contentHash: previous.contentHash,
      updatedAt: previous.updatedAt,
      metrics: previous.metrics,
    })
  }
  while (renderHistory.size > RENDER_HISTORY_LIMIT) renderHistory.delete(renderHistory.keys().next().value)
  const state = {
    root: normalizedRoot,
...
async function hydratePreviewState(root, previewRel) {
  const existing = stateFor(root, previewRel)
  if (existing) return existing
  try {
    const { abs } = resolveUnderJobhunt(path.normalize(root), previewRel)
    const html = await fs.readFile(abs, 'utf8')
    const renderId = /data-render-id="([^"]*)"/.exec(html)?.[1]
    const contentHash = /data-content-hash="([^"]*)"/.exec(html)?.[1]
    if (renderId && contentHash) return rememberPreview(root, previewRel, { renderId, contentHash }, { activate: false })
  } catch {
    // A missing or partially written preview is handled by the caller as a
    // pending render; status must remain read-only and never create files.
  }
  return null
}
      ...
      try {
        ...
        if (body.action === 'rename') {
          ...
          if (!version && body.resumePath) {
            ...
            version = makeVersionRecord({
              ...
              content,
              presentation: versionPresentationSnapshot({ templateId: body.templateId || presentation.activeTemplateId, presentation, resumePath }),
            })
        ...
        const result = await withWorkspaceLock(root, async () => {
          ...
          const template = body.templateId
            ? applyPresentationOverride(await loadTemplate(root, body.templateId), presentation, body.templateId, resumePath)
            : undefined
          const rendered = await renderPreviewUnlocked(root, {
            resumePath,
            outPath: previewPath,
            templateSpec: template,
            initialIconTuning: body.templateId ? getPresentationOverride(presentation, body.templateId, resumePath).iconTuning || {} : {},
          })
          const version = makeVersionRecord({
            ...
            previewPath,
            content,
            presentation: versionPresentationSnapshot({ templateId: body.templateId, presentation, resumePath, template }),
    ...
    async handler(req, res) {
      try {
        ...
        const { saved, rendered } = await withWorkspaceLock(root, async () => {
          ...
          const presentation = await loadPresentation(root)
          const template = body.templateId
            ? applyPresentationOverride(await loadTemplate(root, body.templateId), presentation, body.templateId, resumeRel)
            : undefined
          const rendered = await renderPreviewUnlocked(root, {
            resumePath: resumeRel,
            outPath: previewRel,
            templateSpec: template,
            initialIconTuning: body.templateId ? getPresentationOverride(presentation, body.templateId, resumeRel).iconTuning || {} : {},
          })
  ...
  disposers.push(ctx.webServer.register({
    ...
    async handler(req, res) {
      try {
        ...
        const currentState = currentPreview
          ? (await hydratePreviewState(root, currentPreview)) || state
          : state
        sendJson(res, 200, {
          sessionId,
          root,
          ...
          renderId: currentState?.renderId || null,
          contentHash: currentState?.contentHash || null,
    ...
    async handler(req, res) {
      try {
        ...
        const presentation = draft.templateId
          ...
          : savedPresentation
        const isActiveDraftTemplate = !draft.templateId
          || !savedPresentation?.activeTemplateId
          || savedPresentation.activeTemplateId === draft.templateId
        const html = await renderPreviewHtml(root, {
          resumePath: draft.resumeRel,
          resumeContent: draft.content,
          outPath: draft.previewRel,
          templateSpec: draft.templateId
            ? applyPresentationOverride(await loadTemplate(root, draft.templateId), presentation, draft.templateId, draft.resumeRel)
            : undefined,
          initialIconTuning: draft.templateId ? getPresentationOverride(presentation, draft.templateId, draft.resumeRel).iconTuning || {} : {},
        })
        // Draft previews are ephemeral. A late draft from an old template
        // must not overwrite the active render identity used by status and
        // metrics polling.
        if (isActiveDraftTemplate) rememberPreview(draft.root, draft.previewRel, html)
  ...
  disposers.push(ctx.webServer.register({
    ...
    async handler(req, res) {
      try {
        ...
        if (body.action === 'save') {
          const parsed = typeof body.templateJson === 'string' ? JSON.parse(body.templateJson) : body.templateJson
          if (body.replaceExisting === true && body.confirmImpact !== true) return sendJson(res, 409, { saved: false, conflict: true, error: '覆盖已有模板需要明确确认影响范围；请另存为新模板' })
          return sendJson(res, 200, { saved: true, ...(await withWorkspaceLock(root, () => saveTemplate(root, parsed, { replaceExisting: body.replaceExisting === true, sourceTemplateId: body.sourceTemplateId }))) })
        }
        if (body.action === 'copy') {
        ...
        if (body.action === 'restore-version') {
          return sendJson(res, 200, { restored: true, ...(await withWorkspaceLock(root, () => restoreTemplateVersion(root, body.id, body.versionId))) })
        }
    ...
    async handler(req, res) {
      try {
        ...
        if (req.method === 'POST') {
          ...
          const metricState = metricEntry(state, { renderId: receivedRenderId, contentHash: receivedContentHash })
          if (!metricState) {
            return sendJson(res, 409, {
          ...
          metricState.metrics = {
            renderId: metricState.renderId,
            contentHash: metricState.contentHash,
            previewRel: metricState.previewRel,
            root: metricState.root,
            metrics: body.metrics,
            updatedAt: new Date().toISOString(),
          }
        }
        const requestedRoot = url.searchParams.get('root')
        const requestedPreview = normalizePreviewRel(url.searchParams.get('preview') || url.searchParams.get('path'))
        if (req.method === 'GET' && requestedRoot && requestedPreview) {
          await hydratePreviewState(defaultRoot(requestedRoot, url.searchParams.get('sessionId')), requestedPreview)
        }
        const latest = getLatestMetrics(requestedRoot, requestedPreview)
    ...
    async handler(req, res) {
      try {
        ...
        if (templateId) {
          ...
          // A preview request can arrive late from a stale iframe after the
          // user has already bound another template to this delivery
          // version. It is still safe to render that response for the
          // requester, but it must not replace the workspace's active
          // render identity and trigger the status-poll reload loop.
          const isActiveTemplate = isThumbnail
          ...
          const rendered = await renderPreviewHtml(normalizedRoot, {
              ...
              templateSpec: isThumbnail ? template : applyPresentationOverride(template, presentation, templateId, sourcePathForPreview(rel)),
              initialIconTuning: isThumbnail ? {} : getPresentationOverride(presentation, templateId, sourcePathForPreview(rel)).iconTuning || {},
          ...
          if (!isThumbnail) {
            const previous = stateFor(normalizedRoot, rel)
            const sameEffectiveContent = previous?.contentHash && previous.contentHash === rendered.contentHash
            const effectiveRenderId = sameEffectiveContent ? previous.renderId : rendered.renderId
            const effectiveRendered = sameEffectiveContent ? { ...rendered, renderId: effectiveRenderId } : rendered
            // The metrics payload is embedded in the returned HTML. Keep it
            // aligned with the identity stored in previewState, otherwise a
            // repeated iframe GET can be rejected as a stale render.
            html = sameEffectiveContent
              ? rendered.html.replace(`data-render-id="${rendered.renderId}"`, `data-render-id="${effectiveRenderId}"`)
              : rendered.html
            if (isActiveTemplate) rememberPreview(normalizedRoot, rel, effectiveRendered)
          } else {
            html = rendered.html
          }
          } else {
