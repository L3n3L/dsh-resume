...
import { getTemplatePreset, listTemplatePresets } from './template-presets.js'
...
export function registerPreviewRoutes(ctx) {
  ...
  disposers.push(ctx.webServer.register({
    ...
    async handler(_req, res) {
      sendJson(res, 200, { templates: listTemplatePresets() })
    },
  }))
      ...
      try {
        ...
        const html = templateId
          ? (await renderPreviewHtml(normalizedRoot, {
              ...
              templateSpec: getTemplatePreset(templateId),
            })).html
