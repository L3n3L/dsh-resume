...
const FAMILY_BY_PRESET = {
  ...
  'business-ledger-plus': 'business-timeline',
...
const GALLERY_PRESET_IDS = Object.freeze([
  'campus-standard',
  'portrait-profile',
  'magazine-feature',
  'geek-lab',
...
const COMPOSITION_BY_PRESET = Object.freeze({
  'campus-standard': { page: 'stack', header: 'standard', section: 'line', entry: 'stack', meta: 'inline', skills: 'chips' },
  'business-ledger-plus': { page: 'stack', header: 'hero', section: 'badge', entry: 'timeline', meta: 'split', skills: 'list' },
  'portrait-profile': { page: 'stack', header: 'hero', section: 'badge', entry: 'stack', meta: 'inline', skills: 'chips' },
  'magazine-feature': { page: 'grid', header: 'hero', section: 'line', entry: 'stack', meta: 'inline', skills: 'chips' },
  'geek-lab': { page: 'stack', header: 'hero', section: 'badge', entry: 'timeline', meta: 'split', skills: 'list' },
  'case-study': { page: 'stack', header: 'hero', section: 'badge', entry: 'stack', meta: 'split', skills: 'chips' },
})
...
const VALID_PRESETS = PRESETS.map((preset) => assertTemplateSpec({
  ...
  composition: { ...COMPOSITION_BY_PRESET[preset.id], ...(preset.composition || {}) },
  metadata: {
    ...(preset.metadata || {}),
...
function compositionForLegacyTemplate(template) {
  const renderer = template?.renderer || ''
  const page = template?.layout?.mode === 'two-column'
    ? 'split'
    : renderer === 'portfolio-grid'
      ? 'grid'
      : 'stack'
  const hero = new Set(['business-timeline', 'midnight-terminal', 'editorial', 'magazine-feature', 'portrait-profile', 'heading-stack', 'social-profile']).has(renderer)
  const timeline = new Set(['business-timeline', 'technical-timeline', 'academic', 'midnight-terminal', 'sidebar-signal', 'chronicle-rail', 'geek-lab']).has(renderer)
  const badge = new Set(['business-timeline', 'midnight-terminal', 'portrait-profile', 'metrics-board', 'color-block', 'case-study', 'social-profile']).has(renderer)
  return {
    page,
    header: hero ? 'hero' : 'standard',
    section: badge ? 'badge' : 'line',
    entry: timeline ? 'timeline' : 'stack',
    meta: timeline || page === 'split' ? 'split' : 'inline',
    skills: ['business-timeline', 'technical-timeline', 'academic', 'midnight-terminal', 'geek-lab'].includes(renderer) ? 'list' : 'chips',
  }
}
...
export async function listAvailableTemplates(root) {
  ...
  for (const entry of entries.filter((item) => item.isFile() && item.name.toLowerCase().endsWith('.json'))) {
    try {
      ...
      // The active catalog is composition-only. Old workspace templates stay
      // on disk for recovery, but cannot re-enter the new template library.
...
export async function migrateTemplate(root, id, newId = id) {
  const source = await loadTemplate(root, id)
  if (!source) throw new Error(`template not found: ${id}`)
  if (source.renderer === 'composition') {
    return { migrated: false, template: source, message: 'template already uses composition' }
  }
  const next = {
    ...source,
    id: newId,
    renderer: 'composition',
    composition: compositionForLegacyTemplate(source),
    metadata: {
      ...(source.metadata || {}),
      migratedFromRenderer: source.renderer,
      migratedBy: 'dsh-template-migration',
    },
  }
  return {
    migrated: true,
    fromRenderer: source.renderer,
    ...(await saveTemplate(root, next)),
  }
}
...
export async function saveTemplate(root, input) {
  ...
  if (result.value.renderer !== 'composition') {
    throw new Error('template renderer must be composition for new templates; use jobhunt_template_migrate for an existing legacy template')
  }
  assertCustomTemplateId(result.value.id)
