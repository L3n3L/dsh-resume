...
import { resolveUnderJobhunt, writeJobhuntFile } from './workspace.js'
...
async function recordTemplateVersion(root, current, templateCss = '') {
  ...
  const rel = `.dsh-resume/history/templates/${current.id}/${stamp}.json`
  await writeJobhuntFile(root, rel, `${JSON.stringify({ ...current, templateCss }, null, 2)}\n`)
  return rel
}
...
export async function saveTemplate(root, input, { replaceExisting = false, sourceTemplateId } = {}) {
  ...
  await writeJobhuntFile(root, rel, `${JSON.stringify(persisted, null, 2)}\n`)
  if (nextCss) {
    await writeJobhuntFile(root, `templates/${result.value.id}.css`, nextCss)
...
export async function copyTemplate(root, sourceId, newId, name) {
  ...
  const saved = await saveTemplate(root, next)
...
export async function listTemplateVersions(root, id) {
  ...
  const versions = await Promise.all(entries
    ...
    .map(async (entry) => {
      const pathValue = `.dsh-resume/history/templates/${id}/${entry.name}`
      try {
        const snapshot = JSON.parse(await fs.readFile(resolveUnderJobhunt(root, pathValue).abs, 'utf8'))
...
export async function restoreTemplateVersion(root, id, versionId) {
  ...
  const { abs } = resolveUnderJobhunt(root, version.path)
