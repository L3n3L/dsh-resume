...
export const WORKSPACE_META_DIRNAME = '.dsh-workspace'
export const WORKSPACE_META_FILENAME = 'workspace.json'
...
const SEED_FILES = [
  ...
  ['resume.layout.json', `${JSON.stringify({
    schemaVersion: 1,
    mode: 'auto',
    regions: { main: ['profile', 'education', 'experience', 'projects', 'skills', 'awards', 'links'] },
    blocks: [
...
export const DEMO_RESUME = [
  '# 林知远',
  '',
  '前端开发工程师（2027 届校招） | 北京 / 杭州 | [icon:phone] 158-0000-1234 | [icon:email] lin.zhiyuan@example.com | [icon:github] github.com/example/lin-zhiyuan',
  '',
  '## 教育经历',
  '',
  ...
  '## 专业技能',
  '',
  '- **前端**：[icon:html5] HTML5 / [icon:css3] CSS3 / [icon:javascript] JavaScript / [icon:typescript] TypeScript；[icon:react] React、[icon:vuejs] Vue、[icon:vite] Vite、响应式布局',
  '- **工程**：[icon:nodejs] Node.js、[icon:git] Git、Linux、Vitest、Playwright、Chrome DevTools、[icon:githubactions] CI 基础配置',
  '- **服务与数据**：REST API、SSE、WebSocket、[icon:mysql] SQL；了解 [icon:flask] Flask、[icon:fastapi] FastAPI、[icon:docker] Docker',
...
function normalizeWorkspaceRoot(root) {
  return path.normalize(path.resolve(String(root || '')))
}
...
export async function resolveWorkspaceInput(input) {
  const requestedRoot = normalizeWorkspaceRoot(input)
  const directResume = path.join(requestedRoot, 'resume.md')
  const nestedRoot = path.join(requestedRoot, JOBHUNT_DIRNAME)
  const nestedResume = path.join(nestedRoot, 'resume.md')
  const nestedManifest = path.join(nestedRoot, WORKSPACE_META_DIRNAME, WORKSPACE_META_FILENAME)
  if (path.basename(requestedRoot).toLowerCase() !== JOBHUNT_DIRNAME
      && !(await pathExists(directResume))
      && await isDirectory(nestedRoot)
      && (await pathExists(nestedResume) || await pathExists(nestedManifest))) {
    return { root: nestedRoot, requestedRoot, redirected: true }
  }
  return { root: requestedRoot, requestedRoot, redirected: false }
}

function workspaceManifestPath(root) {
  return path.join(normalizeWorkspaceRoot(root), WORKSPACE_META_DIRNAME, WORKSPACE_META_FILENAME)
}

export async function readWorkspaceManifest(root) {
  try {
    const raw = await fs.readFile(workspaceManifestPath(root), 'utf8')
    const parsed = JSON.parse(raw)
    if (!parsed || typeof parsed !== 'object' || typeof parsed.id !== 'string' || !parsed.id.trim()) return null
    return parsed
  } catch {
    return null
  }
...
export async function ensureWorkspaceManifest(root, name = '') {
  const normalizedRoot = normalizeWorkspaceRoot(root)
  const existing = await readWorkspaceManifest(normalizedRoot)
  if (existing) {
    const updated = existing.root === normalizedRoot
      ? existing
      : { ...existing, root: normalizedRoot, updatedAt: new Date().toISOString() }
    if (updated !== existing) await fs.writeFile(workspaceManifestPath(normalizedRoot), `${JSON.stringify(updated, null, 2)}\n`, 'utf8')
    return updated
  }
  const manifest = {
    schemaVersion: 1,
    id: randomUUID(),
    name: String(name || path.basename(normalizedRoot) || '简历工作区'),
    root: normalizedRoot,
    createdAt: new Date().toISOString(),
  }
  const dir = path.join(normalizedRoot, WORKSPACE_META_DIRNAME)
  await fs.mkdir(dir, { recursive: true })
  const target = workspaceManifestPath(normalizedRoot)
  ...
  try {
    ...
    const concurrent = await readWorkspaceManifest(normalizedRoot)
    if (concurrent) return concurrent
    await fs.writeFile(target, `${JSON.stringify(manifest, null, 2)}\n`, 'utf8')
  }
  return (await readWorkspaceManifest(normalizedRoot)) || manifest
}

export async function getWorkspaceInfo(root) {
  const normalizedRoot = normalizeWorkspaceRoot(root)
  let exists = false
  let directory = false
  try {
  ...
  const manifest = await readWorkspaceManifest(normalizedRoot)
  let initialized = false
  try {
    await fs.access(path.join(normalizedRoot, 'resume.md'))
    initialized = true
  } catch {}
  return {
    root: normalizedRoot,
    ...
    workspaceId: manifest?.id || null,
    workspaceName: manifest?.name || path.basename(normalizedRoot) || '简历工作区',
    manifestPath: workspaceManifestPath(normalizedRoot),
  }
}

async function assertWorkspaceTarget(root, abs) {
  const normalizedRoot = normalizeWorkspaceRoot(root)
  const rootReal = await fs.realpath(normalizedRoot)
  ...
  while (existingParent !== path.dirname(existingParent)) {
    try {
      ...
      if (parentReal !== rootReal && !parentReal.startsWith(rootPrefix)) throw new Error(`workspace path escapes root: ${abs}`)
      break
    } catch (error) {
  ...
  try {
    ...
    if (targetReal !== rootReal && !targetReal.startsWith(rootPrefix)) throw new Error(`workspace path escapes root: ${abs}`)
  } catch (error) {
    if (error?.code !== 'ENOENT') throw error
  }
}
...
/**
 * Resolve the app-managed default without breaking an existing installation
 * that already has the legacy <session-cwd>/jobhunt folder.
 */
export function resolveDefaultWorkspaceRoot(exec) {
  const legacyRoot = path.normalize(path.join(resolveSessionCwd(exec), JOBHUNT_DIRNAME))
  if (fsSync.existsSync(path.join(legacyRoot, 'resume.md'))) return legacyRoot
  const dshHome = process.env.DSH_HOME
  if (dshHome) return path.normalize(path.join(dshHome, 'dsh-resume', 'default'))
  return legacyRoot
}
...
export function resolveJobhuntRoot(exec, rootDir) {
  ...
  return resolveDefaultWorkspaceRoot(exec)
}
...
export async function listJobhunt(root) {
  ...
  async function walk(dir, prefix = '') {
    ...
    for (const ent of entries.sort((a, b) => a.name.localeCompare(b.name))) {
      if (ent.name === WORKSPACE_META_DIRNAME) continue
...
export async function readJobhuntFile(root, relPath) {
  ...
  await assertWorkspaceTarget(root, abs)
...
export async function writeJobhuntFile(root, relPath, content) {
  ...
  await fs.mkdir(path.normalize(root), { recursive: true })
  await assertWorkspaceTarget(root, abs)
  await fs.mkdir(path.dirname(abs), { recursive: true })
  const tempPath = `${abs}.${process.pid}.${randomUUID()}.tmp`
  try {
