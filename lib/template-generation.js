import { normalizeTemplateSpec, validateCssText, validateTemplateSpec, TEMPLATE_DEFAULTS } from './template-schema.js'
...
function rendererForBrief(brief) {
  ...
  if (brief.audience === 'academic') return 'academic'
  if (brief.layout === 'two-column') return brief.family === 'portfolio-grid' || brief.audience === 'design' || brief.audience === 'product' ? 'portfolio-grid' : 'split-sidebar'
  if (brief.tone === 'minimal') return 'swiss-grid'
  if (brief.tone === 'editorial') return 'editorial'
  if (brief.tone === 'terminal') return 'midnight-terminal'
  if (brief.family === 'business-timeline') return 'business-timeline'
  if (brief.tone === 'technical' || brief.audience === 'engineering') return 'technical-timeline'
  return 'clean-single'
}
...
export function generateTemplateCandidate(input = {}) {
  ...
  const ir = renderer === 'portfolio-grid'
  ...
  return {
    ...
    rationale: [
      ...
      `使用 ${rendererForBrief(brief)} Renderer，保证候选模板有真实结构差异`,
      `${family.name}主题家族，模块使用稳定的语义预设而不是自由拼接 CSS`,
    ],
    nextSteps: ['用 jobhunt_template_save 入库（用户已明确要求创建或应用时无需再次确认）', '用 jobhunt_render 生成预览', '用 jobhunt_layout_metrics 读取真实 A4 指标'],
  }
}
