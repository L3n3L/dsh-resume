export const RESUME_GUIDE_VERSION = '1.9.0'
...
4. 用户明确要求保存/应用后才写入；写入后优先重新读取、检查、渲染，并用真实预览指标判断页数、溢出、逐页密度、页间差异和可读性。页数对了但任一页明显空、页间失衡或逐页密度缺失，均是未完成，必须继续调整模板承载/流向/参数并重新验收，不能结束任务，也不能用装饰性空内容填充。密度验收默认要求每页占用率至少 90%，多页之间占用率最大差异不超过 8 个百分点；88.7% 或 80.9% 这类页面仍视为不合格。最终必须调用 resume_finalize；服务端未返回 accepted/completionAllowed=true 时，不得结束任务或声称合格。
MCP 写入默认只是当前草稿，不会因为每次微调自动创建投递副本。只有用户明确要求“保存投递版/另存为版本”时，才调用 resume_save_version；投递副本要求当前版本已经通过 resume_finalize，保存后仍要对新路径重新验收。
...
The default target is one A4 page; if the user explicitly requests another count, pass targetPages to resume_prepare. A matching page count is not enough: missing per-page density, a materially empty page, or a large multi-page imbalance is a hard failure. Continue the revision loop until resume_finalize returns accepted=true and completionAllowed=true.
The default one-page rule above is superseded by an explicit targetPages value: a two-page task must be accepted only at exactly two readable, balanced pages. A delivery copy is not a bypass; resume_save_version(mode=copy) is valid only after resume_finalize accepts the current version.
...
const GUIDE_SECTIONS = {
  workflow: [
    ...
    { step: 9, tools: ['resume_metrics'], when: 'Check layout after the DSH browser preview has measured it.', writes: false, note: 'Standalone MCP may return pending; never invent page count, overflow, whitespace, or page balance. A page-count match without complete per-page density is not a pass. On sparse or unbalanced metrics, continue with presentation_save/template_copy, then render and measure again.' },
    { step: 10, tools: ['resume_finalize'], when: 'Before ending a resume task or claiming a version is qualified/deliverable.', writes: false, note: 'This is the server-side completion gate. It returns accepted only for the current checked content, current render, matching per-page browser metrics, the requested readable A4 page count, balanced density, and no overflow.' },
    { step: 11, tools: ['resume_save_version'], when: 'The user explicitly asks to save as or create a delivery version after the candidate has passed resume_finalize.', writes: true, note: 'A delivery copy is only valid after the current version passes resume_finalize; after saving, the returned version path is a new draft identity and must run check/render/metrics/finalize again.' },
  ],
  modes: {
  ...
  permissions: {
    ...
    mutationTools: ['resume_init', 'resume_write', 'resume_render', 'resume_save_version', 'template_generate', 'template_save', 'template_copy', 'template_restore', 'layout_save', 'presentation_save', 'template_autotune'],
