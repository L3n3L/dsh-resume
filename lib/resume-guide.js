export const RESUME_GUIDE_VERSION = '1.3.0'
...
Default to read-only work: resume_read, resume_check, template_list, icon_list, and layout_validate do not write source files. Only call resume_write after the user explicitly asks to save/apply a content change. The usual flow is resume_read → resume_check → icon_list → (explicit approval) resume_write → resume_render. Improve wording from the user's evidence: use stronger verbs, clearer ownership, and outcome-oriented framing; do not fabricate facts, numbers, scope, responsibilities, or technologies. Protect evidence atoms before shortening. For campus resumes, one A4 page is a preference, not permission to make content incomplete or unreadable. Before drafting, scan relevant employer, school, project, platform, repository, and contact entities; use icon_list to find exact registered brand tokens, use an exact match by default when it improves scanability, and omit the brand icon when no exact match exists. Never substitute a similar brand or invent a slug. Use resume_render for a preview; A4 page metrics belong to the DSH browser preview and must not be guessed.`
