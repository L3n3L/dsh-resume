...
MCP 写入默认只是当前草稿，不会因为每次微调自动创建投递副本。只有用户明确要求“保存投递版/另存为版本”时，才调用 resume_save_version；该工具会把内容、模板及排版快照一起绑定，并要求对新路径重新验收。
...
const GUIDE_SECTIONS = {
  workflow: [
    ...
    { step: 11, tools: ['resume_save_version'], when: 'The user explicitly asks to save as or create a delivery version after reviewing the candidate.', writes: true, note: 'Persists content, template, template lineage, and presentation together; it does not run automatically for every draft mutation. Re-run check/render/metrics/finalize for the returned version path.' },
  ],
  modes: {
