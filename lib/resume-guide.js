...
4. 用户明确要求保存/应用后才写入；写入后优先重新读取、检查、渲染，并用真实预览指标判断页数、溢出、逐页密度、页间差异和可读性。页数对了但任一页明显空、页间失衡或逐页密度缺失，均是未完成，必须继续调整模板承载/流向/参数并重新验收，不能结束任务，也不能用装饰性空内容填充。最终必须调用 resume_finalize；服务端未返回 accepted/completionAllowed=true 时，不得结束任务或声称合格。
