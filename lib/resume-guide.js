export const RESUME_GUIDE_VERSION = '1.7.1'
...
4. 用户明确要求保存/应用后才写入；写入后优先重新读取、检查、渲染，并用真实预览指标判断页数、溢出、留白和可读性。若选中了模板，先在该模板基线上完成参数或结构改造，再考虑其他方案。验证未完成时仍可继续用户明确要求的下一轮迭代，但不得把未经验证的候选声称为“已完成可投递”。
...
const GUIDE_SECTIONS = {
  ...
  permissions: {
    readOnlyTools: ['resume_prepare', 'resume_read', 'resume_check', 'resume_metrics', 'layout_validate', 'template_list', 'template_family_list', 'template_validate', 'template_versions', 'icon_list'],
